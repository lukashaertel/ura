 ...
 */
infix fun <X, T : X, U, V> Choice<T, U>.outerMapLeft(block: (T) -> Choice<V, U>) =
 ...
 */
infix fun <X, T, U : X, V> Choice<T, U>.outerMapRight(block: (U) -> Choice<T, V>) =
        if (isRight) block(right) else Choice<V, U>(true, left)

/**
 * Maps the left value to a value of a new left type.
 */
infix fun <X, T : X, U, V> Choice<T, U>.mapLeft(block: (T) -> V) =
        Choice<V, U>(isLeft, if (isLeft) block(left) else it)

/**
 * Maps the right value to a value of a new right type.
 */
infix fun <X, T, U : X, V> Choice<T, U>.mapRight(block: (U) -> V) =
